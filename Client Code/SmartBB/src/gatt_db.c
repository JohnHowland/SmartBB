...
uint8_t Add_Chat_Service(void)
{
  ...
  D973F2E2-B19E-11E2-9E96-0800200C9A66
	...
	ret =  aci_gatt_add_char(chatServHandle, UUID_TYPE_128, &char_uuid, 20, CHAR_PROP_READ|CHAR_PROP_NOTIFY, ATTR_PERMISSION_NONE, GATT_NOTIFY_ATTRIBUTE_WRITE,
			16, 1, &TXCharHandle);
	if (ret != BLE_STATUS_SUCCESS) goto fail;
...
void Attribute_Modified_CB(uint16_t handle, uint16_t data_length, uint8_t *att_data)
{
	if(handle == RXCharHandle + 1)
	{
		...
		if(att_data[0] == 'U')
			device_state = DEVICE_UNLOCK_STATE;
		if(att_data[0] == 'L')
			device_state = DEVICE_LOCK_STATE;
