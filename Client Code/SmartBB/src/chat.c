...
uint8_t CHAT_DeviceInit(void)
{
	...
	uint8_t name[] = {'S', 'm', 'a', 'r', 't', 'B', 'B', ' ', 'L', 'o', 'c', 'k'};
	...
	/* GAP Init */
	ret = aci_gap_init(role, 0x00, (sizeof(name)/sizeof(uint8_t)), &service_handle,
...
int updateTxCharacteristic(uint8_t response)
{
	uint8_t zeros[1] = {0x59};
	/* Set the device name */
	ret = aci_gatt_update_char_value_ext(0,chatServHandle, TXCharHandle,0,1,0, 1, zeros);
	if (ret != BLE_STATUS_SUCCESS)
	{
		printf ("Error in Gatt Update characteristic value 0x%02x\r\n", ret);
	}
...
void APP_Tick(void)
{
	...
	case DEVICE_IDLE_STATE:
		/* Nothing to Do */
		break;
	case DEVICE_UNLOCK_STATE:
		rotateBackwardHalfStep(2000);
		printf ("Unlocking.");
		counter++;
		updateTxCharacteristic(counter);
		device_state = DEVICE_IDLE_STATE;
		break;
	case DEVICE_LOCK_STATE:
		rotateForwardHalfStep(2000);
		printf ("Locking.");
		counter++;
		updateTxCharacteristic(counter);
		device_state = DEVICE_IDLE_STATE;
	...
	//default:
		...
		//ret = BlueNRG_Sleep(SLEEPMODE_CPU_HALT, 0, 0);
		//		if (ret != BLE_STATUS_SUCCESS) {
